...
//!   - **Axum 框架强制要求**：`OpenApiRouter` 和 `routes!` 宏要求状态类型具有 `'static` 约束
//! - **场景**：作为应用服务长期运行，不依赖于短期引用
//!
//! ##### `'static` 约束的重要性验证 🔬
//!
//! 通过实际测试验证了 `'static` 约束的必要性：
//!
//! **❌ 移除 `'static` 约束时的编译错误：**
//! ```text
//! error[E0310]: the parameter type `PR` may not live long enough
//!   --> crates/web-service/src/routes/mod.rs:54:5
//!    |
//! 54 |     OpenApiRouter::new()
//!    |     ^^^^^^^^^^^^^^^^^^ the parameter type `PR` must be valid for the static lifetime...
//!    |
//! help: consider adding an explicit lifetime bound
//!    |
//! 53 | fn routers<PR: ProjectRepositoryTrait + 'static>(state: AppState<PR>) -> OpenApiRouter {
//!    |                                       +++++++++
//! ```
//!
//! **✅ 恢复 `'static` 约束后编译成功**
//!
//! **具体原因分析：**
//! - `OpenApiRouter::new()` 内部要求状态类型具有 `'static` 生命周期
//! - `routes!` 宏生成的路由处理器需要 `'static` 约束
//! - Web 服务器需要长期运行，状态不能依赖于短期引用
//! - 异步请求处理器会被调度到不同线程，需要确保数据在整个程序生命周期内有效
//!
//! ### 实际应用场景 💡
//!
